...
- Tests:
  - `tests/rehype-code-copy-button.test.ts` — exercise the plugin against seven input cases (single `pre.astro-code`, multiple in one tree, empty `pre.astro-code`, non-Shiki bare `<pre>`, ko vs en locale labels, `pre.astro-code` nested inside `<aside class="callout">`, idempotent re-application). Plugin behavior is fully covered by these unit tests; pipeline-integration is verified through the build smoke step described under Verification Target instead of by a heavyweight end-to-end render test, because the existing `post-detail-structure.test.mjs` infra renders Astro components but does not run the full markdown + remark + rehype + Shiki pipeline.
- Documentation:
...
- **Build smoke**: `npm run build` succeeds. `pagefind --site dist` re-indexes without error. The built `dist/posts/ethereum-keystore-encryption/index.html` and `dist/en/posts/ethereum-keystore-encryption/index.html` files each contain at least one occurrence of the `code-block` wrapper class and one occurrence of the `code-copy-button` class. The Korean dist file contains the `aria-label="코드 복사"` attribute and the English dist file contains the `aria-label="Copy code"` attribute. This grep-based check is the integration verification because it confirms (a) the rehype plugin is wired into Astro's pipeline, (b) the plugin runs against real post markdown, and (c) locale handling works end to end. Run as part of the implementation plan and again before any future changes touching the markdown pipeline.
