...
<!-- TODO: 원본 이미지 — 입력한 패스워드를 기반으로 새로운 Derived Key 생성 (Medium 봇 차단으로 자동 추출 실패) -->
...
<!-- TODO: 원본 이미지 — 새로운 Derived Key를 기반으로, 새로운 mac 생성 (Medium 봇 차단으로 자동 추출 실패) -->
...
<!-- TODO: 원본 이미지 — 기존 mac과 비교 (Medium 봇 차단으로 자동 추출 실패) -->

## 복호화

대부분의 작업이 끝났습니다. 이제 위 과정에서 생성된 derived key를 이용하여, KeyStore 파일에 있는 cipher text를 복호화하면 됩니다.

<!-- TODO: 원본 이미지 — KeyStore 파일 내부의 Cipher Text 복호화 (Medium 봇 차단으로 자동 추출 실패) -->
...
**A:** 어찌 보면 당연한 이유입니다. Scrypt의 결과값은 Private Key를 복호화 하는데 사용하는 키 자체이기 때문입니다.
